...
function Nav() {
  ...
  return (
    <header className='flex justify-between items-center p-5 bg-white shadow-md'>
      ...
      {isOpen && (
        <div >
           <ul className='absolute top-16 left-0 w-full bg-white shadow-md lg:hidden flex flex-col items-center gap-5 py-5 text-xl font-bold'>
          <Link to='/' onClick={() => setIsOpen(false)}><li>Home</li></Link>
          <Link to='/projectS' onClick={() => setIsOpen(false)}><li>Shop</li></Link>
          <Link to='/service' onClick={() => setIsOpen(false)}><li>Blog</li></Link>
